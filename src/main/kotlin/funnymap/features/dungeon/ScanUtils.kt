...
object ScanUtils {
    ...
    val secretList: MutableMap<String, MutableList<Secret>> = try {
        Gson().fromJson<MutableMap<String, MutableList<Secret>>?>(
            mc.resourceManager.getResource(
                ResourceLocation("funnymap", "secretlocations.json")
            ).inputStream.bufferedReader(), object : TypeToken<MutableMap<String, MutableList<Secret>>>() {}.type
        ).map { it.key.replace(Regex("-\\d+$"), "").replace("-", " ") to it.value }.toMap().toMutableMap()
    } catch (e: JsonSyntaxException) {
        println("Error parsing FunnyMap secret data.")
        mutableMapOf()
    } catch (e: JsonIOException) {
        println("Error reading FunnyMap secret data.")
        mutableMapOf()
    }
